...
def get_venta_disponible(db: Session, id_articulo: int,id_codbarra: int, bodega_id: int, estado_id: int, id_lista: int = 0):
        # Definimos el query nativo llamando a la función
        query = text("""
            SELECT
                s.stock AS "stock",
                ...
                s.impuesto,
                s.porcentaje
            FROM ventadisponiblexbodega(:param_articulo_id, :param_id_codbarra, :param_bodega_id, :param_estado_id, :param_id_lista) AS s
        """)
