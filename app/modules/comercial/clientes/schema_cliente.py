...
# Esquema para leer la variable Logs
...
class ClienteBase(BaseModel):
    ...
    direccion: str = Field(alias="direccion", max_length=50)
    mail: str = Field(alias="mail", max_length=60)
    activo: bool = Field(alias="activo")
    observacion: str = Field(alias="observacion", max_length=250)
    fecha_mod: Optional[datetime] = Field(alias="fechaMod", default=None)
    logs: List[LogEntry]
    # Solo se envia cuando se registra una persona nueva, o al editar (donde se
    # permiten corregir los datos de la persona ya ligada) - ver create/update_cliente.
    persona: Optional[PersonaBase] = Field(None, alias="persona")
...
# Esquema para la respuesta de /search (usado en ver/editar)
...
class ClientePaginacion(BaseModel):
    id_cliente: int = Field(alias="idCliente")
    cod_tit: str = Field(alias="codigoTitular", max_length=50)
    nom_cliente: str = Field(alias="nomCliente", max_length=100)
    activo: bool = Field(alias="activo")
...
class PaginatedClienteResponse(BaseModel):
    content: List[ClientePaginacion]
    totalElements: int
    totalPages: int
    number: int
    size: int
